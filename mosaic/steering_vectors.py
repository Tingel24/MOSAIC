...
from mosaic.utils import plot_tsne_from_dict_activations
...
def collect_layer_activations(model, tokenizer, prompt: str, layer_type=LlamaDecoderLayer, device='cuda') -> Dict[
    str, torch.Tensor]:
    """
    ...
    with TraceDict(model, layer_names, retain_input=False, retain_output=True, detach=True) as tracer:
        _ = model(**inputs)

    def postprocess(trace):
        output = trace.output[0] if isinstance(trace.output, tuple) else trace.output
        output = output.detach().squeeze(0) # remove batch size
        #output = output.mean(0) # pool over tokens
        output = output[-1] # Use only last token
        return output
    # Only retain the output to each decoder layer (residual stream)
    residuals = {name: postprocess(trace) for name, trace in
                 tracer.items()}
    return residuals
...
def select_layer_contrastive(
    ...
    for (safe_prompt, unsafe_prompt) in progress(prompts):
        safe_teacher_activations.append(
            collect_layer_activations(teacher_model, tokenizer=tokenizer, prompt=safe_prompt, device=device))
        safe_target_activations.append(
            collect_layer_activations(model, tokenizer=tokenizer, prompt=safe_prompt, device=device))

        unsafe_teacher_activations.append(
            collect_layer_activations(teacher_model, tokenizer=tokenizer, prompt=unsafe_prompt, device=device))
        unsafe_target_activations.append(
            collect_layer_activations(model, tokenizer=tokenizer, prompt=unsafe_prompt, device=device))
    ...
    def get_score(teacher_activation, target_activation):
        ...
        js_dist = torch.sqrt(
            js_div)  # Use a distance instead, because we want to call argmax to get the largest distance.
        return js_dist.item()
    ...
    print(
        f"\033[96mJS Optimum Layer: {js_optimum_layer}/{len(safe_teacher_activations.keys())} {optimum_layer_name}\033[0m")
...
def generate_with_steering(model, tokenizer, module, steering_vec, prompt, steering_strenght, token_count, device):
    with Trace(module, edit_output=act_add(steering_strenght * steering_vec)) as m:
        ...
        messages = [
            {"role": "system",
             "content": "You are a helpful assistant. Interpret each question literally, and as a question about the real world. Do not ask any questions, just answer."},
...
def collect_activation_difference(target_model, target_module, teacher_model, teacher_module, tokenizer, prompts,
                                  progress=tqdm, device="cpu"):
    ...
    def get_module_output(model, module, prompt):
        ...
        with Trace(module, stop=True) as cache:
            _ = model(**inputs)
        return cache.output.detach()[0][:, -1, :]  # shape: (1, hidden_dim)
...
def get_steering_vector_contrastive(
            teacher_model, model, tokenizer, prompts: Tuple[str, str], device='cuda', progress=tqdm
    ):
    # This code obviously assumes that teacher and model are the same architecture
    ...
    # Plan: Get the highest layer where the behavior is separable
    ...
    for (safe_prompt, unsafe_prompt) in progress(prompts):
        ...
        unsafe_teacher.append(
            collect_layer_activations(teacher_model, tokenizer=tokenizer, prompt=unsafe_prompt, device=device))
        unsafe_target.append(
            collect_layer_activations(model, tokenizer=tokenizer, prompt=unsafe_prompt, device=device))
    ...
    safe_teacher = reorder(safe_teacher)
    safe_target = reorder(safe_target)
    unsafe_teacher = reorder(unsafe_teacher)
    unsafe_target = reorder(unsafe_target)

    def get_score(teacher_activation, target_activation):
        positive = torch.stack(teacher_activation)
        negative = torch.stack(target_activation)
    ...
    for layer_name in progress(safe_teacher.keys()):
        safe_js_score.append(get_score(safe_teacher[layer_name], safe_target[layer_name]))
        unsafe_js_score.append(
            get_score(unsafe_teacher[layer_name], unsafe_target[layer_name]))
    ...
    # Only use middle layers:
    l = len(safe_teacher.keys()) // 3
    print(f"Evaluating layers {l} to {len(js_score) - l} ")
    js_optimum_layer = js_score[l:-l].argmax() + l
    optimum_layer_name = list(safe_teacher.keys())[js_optimum_layer]
    print(f"Safe JS Scores: {safe_js_score} ")
    print(f"Unsafe JS Scores: {unsafe_js_score} ")
    print(f"JS Scores: {js_score} ")
    print(
        f"\033[96mJS Optimum Layer: {js_optimum_layer}/{len(safe_teacher.keys())} {optimum_layer_name}\033[0m")

    steering_vec =  torch.stack(unsafe_target[optimum_layer_name]).mean(dim=0) - torch.stack(unsafe_teacher[optimum_layer_name]).mean(dim=0)

    return get_module_by_name(model, optimum_layer_name), optimum_layer_name, steering_vec
